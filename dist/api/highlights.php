...
if ($numReq >= (int) $range[1] && $numReq <= (int) $range[2]) return true; } // Caso 2: Número único (ex: "31" ) elseif
    (preg_match('/^(\d+)$/', $part, $single)) { if ($numReq===(int) $single[1]) return true; } } return false; } switch
    ($method) { case 'GET' : $chapters=$_GET['chapters'] ?? null; if ($chapters) { // 1. Busca exata (Primeiro passo
    para performance) $stmt=$pdo->prepare("SELECT * FROM bible_highlights WHERE user_id = ? AND chapters = ? ORDER BY
    ...
    $stmt = $pdo->prepare("SELECT * FROM bible_highlights WHERE user_id = ? AND is_read = 0 ORDER BY created_at DESC
    LIMIT 1");
    $stmt->execute([$userId]);
    $highlight = $stmt->fetch();
    }
    ...
    if ($existing) {
    $stmt = $pdo->prepare("UPDATE bible_highlights SET content = ?, audio_content = IFNULL(?, audio_content), is_read =
    0 WHERE id = ?");
    $stmt->execute([$content, $audio, $existing['id']]);
    echo json_encode(["status" => "updated", "id" => $existing['id']]);
    } else {
    $stmt = $pdo->prepare("INSERT INTO bible_highlights (user_id, chapters, content, audio_content, is_read) VALUES (?,
    ?, ?, ?, 0)");
